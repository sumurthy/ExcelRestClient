...
module ExcelRubyEasy
  module Model
	class TableRow 
		...
		attr_accessor :index, :values, :tableId
		...
		def get_range()
			...
			j = ExcelRubyEasy::HttpAction::doGetRequest (ExcelRubyEasy::Client.excelserver + "Tables('#{@tableId}')/Rows/$/ItemAt(index=#{index})/Range")
